...
urlpatterns = [
    ...
    path(
        "api/budgets/<uuid:budget_id>/members/", BudgetMembers.as_view(), name="budget-add-and-remove-members"
    ),  # request body format: {"members": ["<user id number>", "<another user id number>"]}
    path("api/budgets/<uuid:budget_id>/expenses/", ListAddExpenses.as_view(), name="list-add-budget-expenses"),
    path(
        "api/budgets/<uuid:budget_id>/expenses/<uuid:expense_id>/",
        ExpenseDetail.as_view(),
        name="get-update-remove-expense-details",
    ),
    ...
    ),  # swagger-ui endpoint for API visualization
